(function(window, document) {
    ...
    function getFileFromCanvasData(dataURL) {
        var blobBin = window.atob(dataURL.substr(dataURL.indexOf(',') + 1));
        var array = [];
                ...
                return {
                    ...
                    post: function(scope, element, attrs, ngModel) {
                        ...
                        var uploadFile = function(scope) {
                            ...
                            var frameImgCanvas = document.createElement('canvas');
                            var imgDisplayArea = document.getElementById('htmlcontent');
                            ...
                            if (selectedFile) {
                                ...
                                reader.onload = function(e) {
                                    ...
                                    var frameImgNormal = new Image();
                                    frameImgNormal.src = reader.result;

                                    /*      frameImgNormal.id = "uploadFrameImgNormal";
                                          frameImgNormal.style.width = '100%';
                                          imgDisplayArea.appendChild(frameImgNormal);
                                       */
                                    var ctx = frameImgCanvas.getContext("2d");
                                    var cmpWidth = frameImgNormal.width,
                                        cmpHeight = frameImgNormal.height;
                                    //         console.log('height=' + cmpHeight + ',width=' + cmpWidth);
                                    if (cmpWidth > 800) {
                                        var xRatio = frameImgNormal.width / 800;
                                        cmpWidth = 800;
                                        cmpHeight = cmpHeight / xRatio;
                                    }
                                    if (cmpHeight > 600) {
                                        var yRatio = cmpHeight / 600;
                                        cmpHeight = 600;
                                        cmpWidth = cmpWidth / yRatio;
                                    }

                                    frameImgCanvas.width = cmpWidth;
                                    frameImgCanvas.height = cmpHeight;
                                    ctx.drawImage(frameImgNormal, 0, 0, cmpWidth, cmpHeight);
                                    selectedFile = getFileFromCanvasData(frameImgCanvas.toDataURL());
                                    //         console.log('selectedFile2=' + selectedFile.size);
    ...
    angular.module('ngS3upload').run(['$templateCache', function($templateCache) {
        ...
        $templateCache.put('theme/bootstrap2.html',
            "<div id=\"htmlcontent\" class=\"upload-wrap\">\n" +
            "  <button class=\"btn btn-primary\" style=\"margin-left: 30%;\" ng-click=\"Clear();\" type=\"button\"><span >Choose file</span></button>\n" +
